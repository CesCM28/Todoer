...
def register():
    if request.method == 'POST':
        ...
        error = None
        c.execute(
            "SELECT id FROM dbo.users WHERE username like ?", username
        ).fetchall()
        if not username:
        ...
        if error is None:
            ...
            c.execute("INSERT INTO dbo.users (username, password) VALUES (?, ?)", (username, generate_password_hash(password)))
            db.commit()
